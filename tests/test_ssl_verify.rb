...
if EM.ssl?
  ...
  class TestSslVerifyWithStringCerts < Test::Unit::TestCase
    def setup
      $dir = File.dirname(File.expand_path(__FILE__)) + '/'
      $private_key = File.read("#{$dir}/client.key")
      $cert_chain = File.read("#{$dir}/client.crt")
    end

    module Client
      def connection_completed
        start_tls(:private_key_string => $private_key, :cert_chain_string => $cert_chain)
      end
